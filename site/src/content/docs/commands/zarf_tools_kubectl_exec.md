---
title: zarf tools kubectl exec
description: Zarf CLI command reference for <code>zarf tools kubectl exec</code>.
tableOfContents: false
---

<!-- Page generated by Zarf; DO NOT EDIT -->

## zarf tools kubectl exec
...
```
zarf tools kubectl exec (POD | TYPE/NAME) [-c CONTAINER] [flags] -- COMMAND [args...]
```
...
```
  ...
  zarf tools kubectl exec mypod -- date
  
  # Get output from running the 'date' command in ruby-container from pod mypod
  zarf tools kubectl exec mypod -c ruby-container -- date
  
  # Switch to raw terminal mode; sends stdin to 'bash' in ruby-container from pod mypod
  # and sends stdout/stderr from 'bash' back to the client
  zarf tools kubectl exec mypod -c ruby-container -i -t -- bash -il
  ...
  zarf tools kubectl exec mypod -i -t -- ls -t /usr
  
  # Get output from running 'date' command from the first pod of the deployment mydeployment, using the first container by default
  zarf tools kubectl exec deploy/mydeployment -- date
  
  # Get output from running 'date' command from the first pod of the service myservice, using the first container by default
  zarf tools kubectl exec svc/myservice -- date
```
