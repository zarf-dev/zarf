---
title: zarf tools kubectl config get-contexts
description: Zarf CLI command reference for <code>zarf tools kubectl config get-contexts</code>.
...
```
zarf tools kubectl config get-contexts [(-o|--output=)name)]
```
