---
title: zarf tools kubectl top node
description: Zarf CLI command reference for <code>zarf tools kubectl top node</code>.
...
```
zarf tools kubectl top node [NAME | -l label]
```
...
```
  ...
  -l, --selector string        Selector (label query) to filter on, supports '=', '==', '!=', 'in', 'notin'.(e.g. -l key1=value1,key2=value2,key3 in (value3)). Matching objects must satisfy all of the specified label constraints.
      ...
      --show-swap              Print node resources related to swap memory.
