---
title: zarf tools kubectl events
description: Zarf CLI command reference for <code>zarf tools kubectl events</code>.
...
```
zarf tools kubectl events [(-o|--output=)json|yaml|kyaml|name|go-template|go-template-file|template|templatefile|jsonpath|jsonpath-as-json|jsonpath-file] [--for TYPE/NAME] [--watch] [--types=Normal,Warning]
```
