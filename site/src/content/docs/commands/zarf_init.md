...
```
      ...
      --agent-mutation-mode mutationMode   Whether the Zarf agent mutates resources by default ("opt-out") or only when they or their namespace are labeled with "zarf.dev/agent: mutate" ("opt-in") (default opt-out)
