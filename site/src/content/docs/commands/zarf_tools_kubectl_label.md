---
title: zarf tools kubectl label
description: Zarf CLI command reference for <code>zarf tools kubectl label</code>.
tableOfContents: false
---

<!-- Page generated by Zarf; DO NOT EDIT -->

## zarf tools kubectl label
...
```
zarf tools kubectl label [--overwrite] (-f FILENAME | TYPE NAME) KEY_1=VAL_1 ... KEY_N=VAL_N [--resource-version=version]
```
...
```
  ...
  zarf tools kubectl label pods foo unhealthy=true
  
  # Update pod 'foo' with the label 'status' and the value 'unhealthy', overwriting any existing value
  zarf tools kubectl label --overwrite pods foo status=unhealthy
  
  # Update all pods in the namespace
  zarf tools kubectl label pods --all status=unhealthy
  
  # Update a pod identified by the type and name in "pod.json"
  zarf tools kubectl label -f pod.json status=unhealthy
  
  # Update pod 'foo' only if the resource is unchanged from version 1
  zarf tools kubectl label pods foo status=unhealthy --resource-version=1
  
  # Update pod 'foo' by removing a label named 'bar' if it exists
  # Does not require the --overwrite flag
  zarf tools kubectl label pods foo bar-
```
