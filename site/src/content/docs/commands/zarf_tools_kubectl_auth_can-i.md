---
title: zarf tools kubectl auth can-i
description: Zarf CLI command reference for <code>zarf tools kubectl auth can-i</code>.
tableOfContents: false
---

<!-- Page generated by Zarf; DO NOT EDIT -->

## zarf tools kubectl auth can-i
...
```
zarf tools kubectl auth can-i VERB [TYPE | TYPE/NAME | NONRESOURCEURL]
```
...
```
  ...
  zarf tools kubectl auth can-i create pods --all-namespaces
  
  # Check to see if I can list deployments in my current namespace
  zarf tools kubectl auth can-i list deployments.apps
  
  # Check to see if service account "foo" of namespace "dev" can list pods in the namespace "prod"
  # You must be allowed to use impersonation for the global option "--as"
  zarf tools kubectl auth can-i list pods --as=system:serviceaccount:dev:foo -n prod
  
  # Check to see if I can do everything in my current namespace ("*" means all)
  zarf tools kubectl auth can-i '*' '*'
  
  # Check to see if I can get the job named "bar" in namespace "foo"
  zarf tools kubectl auth can-i list jobs.batch/bar -n foo
  
  # Check to see if I can read pod logs
  zarf tools kubectl auth can-i get pods --subresource=log
  
  # Check to see if I can access the URL /logs/
  zarf tools kubectl auth can-i get /logs/
  
  # Check to see if I can approve certificates.k8s.io
  zarf tools kubectl auth can-i approve certificates.k8s.io
  
  # List all allowed actions in namespace "foo"
  zarf tools kubectl auth can-i --list --namespace=foo
```
