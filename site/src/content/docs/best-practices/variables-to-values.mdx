...
Constants have no direct replacement. A constant is a fixed value a deployer cannot override, whereas any package value can be overridden at deploy time with `--set-values` or `-v`. Depending on why the constant existed, approximate it one of these ways:
