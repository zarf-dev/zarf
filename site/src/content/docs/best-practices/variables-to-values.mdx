...
To verify that this change made no impact on your package run `zarf dev inspect manifests` before your migration with the appropriate variables set. Then run the command the same command with values set post migration. The yaml content should match.
