...
1. The `dos-games` package is easily deployable via `oci://` by running `zarf package deploy oci://zarf-dev/dos-games:1.0.0-$(uname -m) --key=https://zarf.dev/cosign.pub`.
