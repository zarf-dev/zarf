...
You can run [`zarf dev lint <directory>`](/commands/zarf_dev_lint/) to validate against the [`zarf.schema.json`](https://github.com/defenseunicorns/zarf/blob/main/zarf.schema.json), or setup [VSCode](/ref/dev/#vscode) to see errors in real-time.
...
To resolve this, we can add configuration options with [Zarf Variables](/ref/examples/values/#variables-zarf_var_).  For this package we will add a `variables` section to our `zarf.yaml` above `components` that will allow us to setup the user and the blog.
