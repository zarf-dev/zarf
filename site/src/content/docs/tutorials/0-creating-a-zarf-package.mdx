...
To resolve this, we can add configuration options with [Zarf Deploy-Time Variables](/ref/examples/variables/).  For this package we will add a `variables` section to our `zarf.yaml` above `components` that will allow us to setup the user and the blog.
