...
In previous tutorials, we learned how to [create a package](/tutorials/00-creating-a-zarf-package/), [initialize a cluster](/tutorials/01-initializing-a-k8s-cluster/), and [deploy a package](/tutorials/02-deploying-zarf-packages/). In this tutorial, we will leverage all that past work and deploy a fun application onto your cluster.
...
You can publish your own packages for deployment too via `oci://`.  See the [Store and Deploy Packages with OCI](/tutorials/06-publish-and-deploy/) tutorial for more information.
...
If you receive this error when zarf is attempting to deploy the `BASELINE COMPONENT`, this means you have not initialized the kubernetes cluster.  This is one of the prerequisites for this tutorial.  Perform the [Initialize a cluster](/tutorials/01-initializing-a-k8s-cluster/) tutorial, then try again.
