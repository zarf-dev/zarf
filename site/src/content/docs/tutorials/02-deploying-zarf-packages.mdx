...
In this tutorial, we are going to deploy the WordPress package onto your cluster using the package we created in the earlier [create a package](/tutorials/00-creating-a-zarf-package/) tutorial and the cluster we initialized in the [initialize a k8s cluster](/tutorials/01-initializing-a-k8s-cluster/) tutorial. We will be leveraging that past work to go the extra step of deploying an application we packaged onto our cluster with the `zarf package deploy` command.
...
- The [WordPress package created](/tutorials/00-creating-a-zarf-package/)
...
2. You will be presented with a chance to review the SBOMs for the package along with its definition followed by a series of prompts for each variable we setup in the [previous tutorial](/tutorials/00-creating-a-zarf-package/#setting-up-variables).  To confirm package deployment press `y` then `enter` and input a value for each variable when prompted followed by `enter` for them as well.
...
3. Because we included the connect services in the [previous tutorial](/tutorials/00-creating-a-zarf-package/#setting-up-a-zarf-connect-service) we can quickly test our package in a browser with `zarf connect wordpress-blog`.
...
1. Deploy a Kubernetes cluster with the [Creating a K8s Cluster with Zarf](/tutorials/04-creating-a-k8s-cluster-with-zarf/) tutorial.
2. Perform the [Initialize a cluster](/tutorials/01-initializing-a-k8s-cluster/) tutorial.
...
If you receive this error when zarf is attempting to deploy a package, this means you have not initialized the kubernetes cluster.  This is one of the prerequisites for this tutorial.  Perform the [Initialize a cluster](/tutorials/01-initializing-a-k8s-cluster/) tutorial, then try again.
