...
4. We can also explore the resources deployed by our package by running the `zarf tools monitor` command to start [`k9s`](/ref/tools/#k9s). Once you are done, hit `ctrl/control c` to exit.
   ...
   Deploying packages isn't the only way to interact with them in the air gap.  If you would like to quickly inspect a package and it's SBOMs you can use [`zarf package inspect`](/ref/sboms/#viewing-a-packages-sboms) to view them, and if you would like to push resources inside of a Zarf package (i.e. the images in this Wordpress package) to services in the air gap without running a deployment, you can do so with [`zarf package mirror-resources`](/commands/zarf_package_mirror-resources/).
