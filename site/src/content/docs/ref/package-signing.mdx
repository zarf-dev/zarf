...
Keyless signing uses Sigstore's [Fulcio](https://docs.sigstore.dev/certificate_authority/overview/) certificate authority to issue a short-lived signing certificate bound to your OIDC identity (GitHub Actions workflow, Google account, etc.). No private key is generated or stored — the certificate itself proves who signed the package.
