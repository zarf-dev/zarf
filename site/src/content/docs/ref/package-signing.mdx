...
### Sigstore Bundle Format (Recommended)

As of recent Zarf releases, packages are signed using the [Sigstore bundle format](https://docs.sigstore.dev/about/bundle/), stored as `zarf.bundle.sig` within the package. This format provides:
...
**Solution**: Use the `--overwrite` flag to replace the existing signature:
