...
Re-sign packages with legacy signatures using the `zarf package sign --overwrite` command to upgrade to the bundle format. The `--overwrite` flag allows replacing existing signatures.
