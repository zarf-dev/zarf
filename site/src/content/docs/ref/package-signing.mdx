...
Package signing lets consumers verify a package's contents and that it was signed by an expected identity before deployment. Zarf implements signing and verification with [Cosign](https://github.com/sigstore/cosign), supporting key-based signing with a local private key or cloud KMS, and keyless signing with a Sigstore OIDC identity.
...
The signing flags on `zarf package create` and `zarf package publish` are deprecated, but remain supported until their removal in Zarf v1.0.0. Create the package locally, then sign the resulting archive before publishing it. Zarf adds the signature to the package archive and marks the `zarf.yaml` as signed.

```bash
zarf package create . --output ./packages
zarf package sign ./packages/zarf-package-example-amd64.tar.zst \
  --signing-key cosign.key \
  --signing-key-pass <password>
```
