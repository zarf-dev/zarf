...
The behavior of the `zarf-seed-registry` component differs depending on the registry mode, declared by the flag `--registry-mode=nodeport|proxy`. By default, nodeport mode is used for the cluster's first init. Subsequent `zarf init` executions continue using the registry mode that was used on initial init, unless explicitly set. Alternatively, all registry components are skipped when [using an external registry](#using-external-registries).
...
Proxy mode is a newer alternative to nodeport mode to circumvent limitations with NFTables and IPv6 clusters. It has the distinct advantage of securing registry connections over mTLS using automatically generated certificates stored in cluster. A DaemonSet proxy runs [socat](https://pkgs.alpinelinux.org/package/edge/main/x86/socat), this allows the on each node which avoid unencrypted cross-node registry calls. Additionally, Zarf uses the same mTLS certificates to encrypt its calls to the registry.
...
4. The `docker-registry` chart in the `zarf-seed-registry` component is then deployed with values `.proxy.enabled: true`. This creates a long-lived host port or host network socat proxy DaemonSet alongside a Zarf Injector DaemonSet.
5. The docker-registry Deployment and Proxy DaemonSet use the Zarf Injector DaemonSet as their image source. The injector is exposed on each node via host port or host network, allowing the kubelet to pull the required images locally.
