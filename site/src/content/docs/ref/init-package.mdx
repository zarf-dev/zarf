...
The behavior of the `zarf-seed-registry` component differs depending on the registry mode, decided by the init flag `--registry-mode=nodeport|proxy`. Alternatively, the registry components are skipped when [using an external registry](#using-external-registries) with `--registry-mode=external`.
...
1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf determines the [pause image](https://github.com/kubernetes/kubernetes/tree/master/build/pause) to be used by the injector DaemonSet. Since Kubernetes does not expose the cluster's pause image, Zarf looks for an image with the following criteria:
...
An advantage of proxy mode is that it's automatically secured behind mTLS. The proxy DaemonSet uses mTLS certificates stored in the cluster to secure its connection to the registry. Since the proxy runs on each node, the kubelet never transmits unencrypted data outside of each host. Additionally, Zarf uses the same mTLS certificates to encrypt its calls to the registry when in proxy mode.
