...
The `zarf-injector` component copies the injector rust binary to `###ZARF_TEMP###/zarf-injector`, it doesn't launch the injector.

The behavior of the `zarf-seed-registry` component differs depending on the registry mode, decided by the init flag `--registry-mode=nodeport|proxy`

#### NodePort mode

Nodeport mode relies on a short lived injector to create a localhost NodePort service that the registry Deployment in the `zarf-seed-registry` chart can pull from.

:::note

Doing this keeps Zarf cluster agnostic, however does require that the kubelet be able to reach out to a cluster NodePort service, which may require changes to firewall configurations like allowing UDP traffic between nodes if using something like VXLAN tunneling. In addition, IPv6 only clusters and NFTables block access to localhost NodePort services.
...
1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf connects to the cluster and chooses the [pause image](https://github.com/kubernetes/kubernetes/tree/master/build/pause)
  - Because Kubernetes does not expose which pause image is in use, Zarf determines the pause image by choosing the highest version image that meet the following criteria: between major semver versions of 3 or 4, less than 1Mib in size, includes pause in the name.
3. Zarf mounts the `configmaps` that contain the `zarf-injector` binary and the `registry:3` and `alpine/socat` image chunks
4. The `docker-registry` chart in the `zarf-seed-registry` component is then deployed with the `proxy.enabled: true`. This creates a long lived hostPort or hostNetwork proxy DaemonSet alongside a Zarf Injector Daemonset.
5. The registry Deployment and Proxy Daemonset have images pointing to the Zarf Injector Daemonset, which is also exposed through a host port or host network proxy. This allows the Kubelet to pull these images.
...
1a. In NodePort mode the `image.repository` is set to the value of the built-in variable `###ZARF_REGISTRY###`. This means the registry image is pulled from the registry.
1b. In proxy mode the `image.repository` stays pointed to the long lived injector.
