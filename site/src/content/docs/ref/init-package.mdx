...
1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf finds the most likely [pause image](https://github.com/kubernetes/kubernetes/tree/master/build/pause) so that it can spin up an injector DaemonSet. Since Kubernetes does not expose the cluster's pause image, Zarf looks for an image with the following criteria:
...
An advantage of proxy mode is that it's automatically secured behind mTLS. The proxy DaemonSet uses automatically generated mTLS certificates stored in the cluster to secure its connection to the registry. Since the proxy runs on each node, the kubelet never transmits unencrypted traffic across nodes. Additionally, Zarf will use the same mTLS certificates to encrypt its calls to the registry.

:::note

Host port connections do not work on IPv6 clusters, so Zarf will automatically configure the proxy and injector to use host network in these cases.
