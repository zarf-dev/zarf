...
The behavior of the `zarf-seed-registry` component differs depending on the registry mode, declared by the flag `--registry-mode=nodeport|proxy`. By default, nodeport is the registry mode in the cluster's first init. Subsequent `zarf init` executions continue using the registry mode that was used on initial init, unless explicitly set. Alternatively, all registry components are skipped when [using an external registry](#using-external-registries).
...
Proxy mode is a newer alternative to nodeport mode to circumvent limitations with NFTables and IPv6 clusters. It has the distinct advantage of securing registry connections over mTLS using automatically generated certificates stored in the cluster. A DaemonSet proxy runs socat on each node so the kubelet won't make unencrypted cross-node registry calls. Additionally, Zarf uses the same mTLS certificates to encrypt its calls to the registry.
