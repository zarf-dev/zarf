...
However, in an air-gapped environment, these external providers are not available, or exist at different locations from their references within Kubernetes manifests.
...
Want to see a guided `zarf init`? Check out the [Zarf Init tutorial](/tutorials/01-initializing-a-k8s-cluster/).
...
While there is no distro-agnostic method to inject images into a cluster, every cluster has support for `configmaps`. However, the size of a `configmap` is limited to 1MB (technically only limited by whatever is configured in `etcd`, the default is 1MB), and the compressed `registry:3` image is around 18MB (as of this writing). So we split the `registry:3` image into chunks and inject them into the cluster as `configmaps`.
...
> The `zarf-injector` binary exists in its own repository, https://github.com/zarf-dev/zarf-injector, see the [README.md](https://github.com/zarf-dev/zarf-injector/blob/main/README.md) instructions to build and test.

The `zarf-injector` binary is statically compiled and injected into the cluster as a `configmap` along with the chunks of the `registry:3` image. During the `zarf-seed-registry`'s deployment, the `zarf-injector` binary is run in a pod that mounts the `configmaps` and reassembles the `registry:3` image. It then hosts a pull-only Docker registry implemented in Rust so that a real registry can be deployed into the cluster from the hosted `registry:3` image.

:::note

The `registry:3` image and the Zarf Agent image can be configured with a custom init package using the `registry_image_*` and `agent_image_*` templates defined in the Zarf repo's [zarf-config.toml](https://github.com/zarf-dev/zarf/blob/main/zarf-config.toml). This allows you to swap them for enterprise provided / hardened versions if desired such as those provided by [Iron Bank](https://repo1.dso.mil/dsop/opensource/defenseunicorns/zarf/zarf-agent).

:::

#### Zarf Injector

The `zarf-injector` component simply copies the injector Rust binary to `###ZARF_TEMP###/zarf-injector`.

#### Zarf Seed Registry

The behavior of the `zarf-seed-registry` component differs depending on the registry mode, declared by the flag `--registry-mode=nodeport|proxy`. By default, nodeport mode is used for the cluster's first init. Subsequent inits reuse the registry mode stored in cluster state when not explicitly set. Alternatively, all registry components are skipped when [using an external registry](#using-external-registries).

##### NodePort Mode (Default)

When `zarf init` deploys the `zarf-seed-registry` component in NodePort mode, the following happens:

1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf connects to the cluster and grabs a pod that is running an image that is already present in the cluster.
3. Zarf spins up a pod using the existing image, mounts the `configmaps` that contain the `zarf-injector` binary and the `registry:3` image chunks and runs the `zarf-injector` binary.
4. The `zarf-injector` binary reassembles the `registry:3` image and hosts a temporary registry that the cluster can pull from.
...
NodePort mode requires that the kubelet be able to reach out to a cluster NodePort service, which may require changes to firewall configurations like allowing UDP traffic between nodes if using something like VXLAN tunneling. In addition, IPv6-only clusters and NFTables block access to localhost NodePort services.
...
Proxy mode is a newer alternative to nodeport mode to circumvent limitations with NFTables and IPv6-only clusters. It secures registry connections over mTLS using automatically generated certificates stored in cluster. The kubelet calls a DaemonSet proxy, which is running [socat](https://pkgs.alpinelinux.org/package/edge/main/x86/socat), via host port to avoid unencrypted cross-node traffic; Zarf uses the same mTLS client certificates to encrypt its calls to the registry.

When `zarf init` deploys the `zarf-seed-registry` component in proxy mode, the following happens:

1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf finds the most likely [pause image](https://github.com/kubernetes/kubernetes/tree/master/build/pause) to use as the container image in the injector DaemonSet. Since Kubernetes does not expose the cluster's pause image, Zarf looks for an image with the following criteria:
  - Includes pause in the name.
  - Has a major semver version of 3 or 4.
  - Less than 1MiB in size
3. Zarf mounts the `configmaps` that contain the `zarf-injector` binary and the `registry:3` & `alpine/socat` image chunks.
4. The `docker-registry` chart in the `zarf-seed-registry` component is then deployed with values `.proxy.enabled: true`. This creates a long-lived host port or host network Zarf Injector DaemonSet alongside the socat proxy DaemonSet.
5. The docker-registry Deployment and proxy DaemonSet spin up using the injector as their image source.

:::note

Host port connections do not work on IPv6-only clusters, so Zarf will automatically configure the proxy and injector to use host network on these clusters. A pod with host network uses the network namespace of its node. Operators should ensure this is within their acceptable risk tolerance.

:::

#### Zarf Registry

The `zarf-registry` component deploys the long-lived container registry to the cluster.

It leverages the same `docker-registry` chart used in `zarf-seed-registry` but with a few key differences:

1. In NodePort mode, the `image.repository` points to the registry NodePort service deployed by `zarf-seed-registry`. Since this is the same chart, the new registry deployment pulls its image from itself. In proxy mode, the `image.repository` continues to point to the injector.
...
The `zarf-agent` modifies the following [Flux](https://fluxcd.io/flux/) resources: [GitRepository](https://fluxcd.io/flux/components/source/gitrepositories/), [OCIRepository](https://fluxcd.io/flux/components/source/ocirepositories/), & [HelmRepository](https://fluxcd.io/flux/components/source/helmrepositories/) to point to the local Git Server or Zarf Registry. HelmRepositories are only modified if the `type` key is set to `oci`. During the mutation of OCIRepositories, a call is made to the Zarf Registry to determine the media type of the OCI artifact. If the artifact is a helm chart the mutation will __NOT__ include the crc32 hash as including the hash interferes with the Flux deployment of the chart.

The `zarf-agent` modifies [ArgoCD applications](https://argo-cd.readthedocs.io/en/stable/user-guide/application-specification/), [ArgoCD ApplicationSets](https://argo-cd.readthedocs.io/en/stable/user-guide/application-set/), [ArgoCD Repositories](https://argo-cd.readthedocs.io/en/stable/user-guide/private-repositories/), and [ArgoCD AppProjects](https://argo-cd.readthedocs.io/en/stable/operator-manual/project-specification/) to point to the local Git Server.
...
If you would like to adopt pre-existing resources into a Zarf deployment you can use the `--take-ownership` flag on [`zarf package deploy`](/commands/zarf_package_deploy/) to adopt those resources into the Helm Releases that Zarf manages (including namespaces). This will add the requisite annotations and labels to those resources and add the `zarf.dev/agent: mutate` label to any namespace specified by those resources.
...
The agent's mutation behavior is controlled by the `--agent-mutation-policy` flag on `zarf init`. It accepts two values:

- **`all`** (default) — the agent mutates all applicable resources. To exclude a resource, add the label `zarf.dev/agent: ignore` to it or its namespace.
- **`labeled`** — the agent only mutates resources (or resources in namespaces) labeled `zarf.dev/agent: mutate`. All namespaces deployed by Zarf are automatically labeled `zarf.dev/agent: mutate` unless they already carry a `zarf.dev/agent` label.

In both policies, a label on the resource always takes priority over a label on its namespace. A resource labeled `zarf.dev/agent: mutate` in a namespace labeled `zarf.dev/agent: ignore` will be mutated, and vice versa.

Argo CD or Flux resources can be excluded from mutation as a group with the `AGENT_MUTATION_EXCLUSIONS` Zarf package variable. The variable accepts a YAML list containing:
...
You can learn more about creating a custom init package in the [Creating a Custom 'init' Package Tutorial](/tutorials/07-custom-init-packages).
