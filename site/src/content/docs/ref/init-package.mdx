...
Proxy mode is a newer alternative to nodeport mode to circumvent limitations with NFTables and IPv6 clusters. It secures registry connections over mTLS using automatically generated certificates stored in cluster. The kubelet calls a DaemonSet proxy running [socat](https://pkgs.alpinelinux.org/package/edge/main/x86/socat) with host port to avoid unencrypted cross-node traffic; Zarf uses the same mTLS client certificates to encrypt its calls to the registry.
