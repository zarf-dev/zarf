...
By default the registry included in the init package creates a ReadWriteOnce PVC and only schedules onto one node at a time. For smaller and simpler deployments this is usually enough. However, for larger deployments or deployments where nodes will be restarted or updated somewhat frequently, you may want to make the registry highly-available.

This can have some prerequisites, such as having a storage class that supports ReadWriteMany, or being in an environment where you can configure the registry to use an s3 compatible backend. Beyond the backing storage, though, you will need to pass some custom configuration to the registry so that it spreads out across all nodes and has the correct amount of replicas. Below is an example configuration using a ReadWriteMany storage class:
...
Of note is the `REGISTRY_AFFINITY_CUSTOM` variable which overrides the default pod anti-affinity and `REGISTRY_HPA_AUTO_SIZE` which will automatically set the min and max replicas for the registry based on the number of nodes in the cluster. If you would prefer to set the min and max replicas manually you can set `REGISTRY_HPA_MIN` and `REGISTRY_HPA_MAX` to the desired values instead.
