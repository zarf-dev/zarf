...
> The `zarf-injector` binary exists in it's own repository, https://github.com/zarf-dev/zarf-injector, see the [README.md](https://github.com/zarf-dev/zarf-injector/blob/main/README.md) instructions to built and test.
...
{/* FIXME check external flag works how I'd expect */}
...
1. Zarf injects the `zarf-injector` binary and the `registry:3` image chunks into the cluster.
2. Zarf connects to the cluster and determines the [pause image](https://github.com/kubernetes/kubernetes/tree/master/build/pause) to be used by the injector DaemonSet. Because Kubernetes does not expose the cluster's pause image, the pause image is approximated to the highest versioned image in cluster meeting the following criteria: has a major semver version 3 or 4, less than 1Mib in size, includes pause in the name.
