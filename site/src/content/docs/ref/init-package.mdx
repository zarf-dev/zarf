...
The agent can be changed to use an opt-in model through the `zarf init` flag `agent-mutation-mode=opt-in`. Resources will then only be mutated if they are labeled with `zarf.dev/agent: mutate` or are in a namespace with the same label. The resource label always takes priority so a resource labeled with `zarf.dev/agent: mutate` in a namespace labeled `zarf.dev/agent: ignore` will be mutated. All namespaces deployed by Zarf automatically get labeled `zarf.dev/agent: mutate` unless they have a pre-existing  `zarf.dev/agent` label.
