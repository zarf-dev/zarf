...
If the subfield `images` is set within `imageArchives` then Zarf will only specified images will be pulled into the package. If `images` is empty, then Zarf will pull in every recognized image into the package, and populate the `imageArchives.image` field during the build process.
