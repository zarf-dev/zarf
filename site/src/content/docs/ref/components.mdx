...
By default a Zarf package is built for one architecture and each image pulled matches that architecture. To build a single package that deploys on multi-arch clusters, set `metadata.architecture: multi`. Zarf will then pull all available platforms for an image reference; pinning to an index digest is recommended for reproducibility and integrity.
