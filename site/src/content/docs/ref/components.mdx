...
OCI images can also be brought into a package via archives of [OCI layouts](https://specs.opencontainers.org/image-spec/image-layout/). Common ways to create OCI archives are through the [docker save](https://docs.docker.com/reference/cli/docker/image/save/) and [podman save --format=oci-archive](https://docs.podman.io/en/latest/markdown/podman-save.1.html) commands. The Docker engine version must be >= than [25.0.0](https://docs.docker.com/engine/release-notes/25.0/#2500) as this was the release that the save command became compliant with the OCI layout specification.

If the subfield `images` is set within `imageArchives` then Zarf will only pull the specified images into the package. If `imageArchives.images` is empty, then Zarf will pull in every recognized image into the package, and populate the `imageArchives.images` field during the build process.

In order for Zarf to recognize an image its [descriptor](https://specs.opencontainers.org/image-spec/descriptor/?v=v1.1.1) in the OCI-layout index.json must include one of the following annotations: `io.containerd.image.name` or `org.opencontainers.image.ref.name`. `docker save` and `podman save` automatically at least one of these annotations.
