...
OCI images can also be brought into a package via archives of [OCI layouts](https://specs.opencontainers.org/image-spec/image-layout/). The only currently supported archive format is [docker save](https://docs.docker.com/reference/cli/docker/image/save/) command. The Docker engine version must be >= than [25.0.0](https://docs.docker.com/engine/release-notes/25.0/#2500) as this was the release that the save command became compliant with the OCI layout specification. In order for Zarf to consume an image referenced by sha during docker save, the Docker engine must use the [container image store](https://docs.docker.com/engine/storage/containerd/), which is the default in 29.0.0.

If the subfield `images` is set within `imageArchives` then Zarf will only pull in the specified images from that tar file. If `images` is empty Zarf will pull in every image in the archive, and populate the field during the build process.
