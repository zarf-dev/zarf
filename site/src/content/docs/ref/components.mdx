...
If the subfield `images` is set within `imageArchives` then Zarf will only pull in the specified images from that tar file. If `images` is empty Zarf will pull in every image in the archive.
