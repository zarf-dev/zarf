...
If the sub-field `imageArchives.images` is set then then Zarf will only pull the specified images into the package. If this field is empty, then Zarf will pull in every recognized image into the package, and populate `imageArchives.images` during the build process.

In order for Zarf to recognize an image its [descriptor](https://specs.opencontainers.org/image-spec/descriptor/?v=v1.1.1) in the OCI-layout index.json must include the annotation "io.containerd.image.name" or "org.opencontainers.image.ref.name". `docker save` and `podman save` will automatically add at least one of these annotations.
