...
OCI images can also be brought into a package via archives of [OCI layouts](https://specs.opencontainers.org/image-spec/image-layout/). The most common way to create an image archive is through the command `docker save my-image:0.0.1 -o image.tar`. The Docker engine version must be >= than [25.0.0](https://docs.docker.com/engine/release-notes/25.0/#2500) as this was the release that the save command became compliant with the OCI layout specification.
