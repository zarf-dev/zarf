...
By default a Zarf package is built for one architecture and each image pulled matches that architecture. To build a single package that deploys on multi-arch clusters, set `metadata.architecture: multi`. Every image in a `multi` package must be pinned by digest. The digest should usually point to an OCI image index so multiple platforms for each image can be pulled, but can also point to a single-platform manifest.
...
Data injections are deprecated and will be removed in the next schema version. Please migrate to using OCI image-based data delivery instead. See the [Data Injections Migration Guide](/best-practices/data-injections-migration/) for instructions.
