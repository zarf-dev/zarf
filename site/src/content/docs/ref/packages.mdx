...
Documentation can be stored in a package using the top level `documentation` field. Package consumers can view this documentation using [zarf package inspect documentation](/commands/zarf_package_inspect_documentation). These docs can be useful for communicating configuration options, changelogs, or application specific information. View an example use case of documentation in the [dos-games](/ref/examples/dos-games) package.
