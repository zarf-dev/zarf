...
Zarf packages can include documentation files that provide important information to package deployers. This is particularly useful for communicating configuration options, deployment instructions, security considerations, or any other information that users need to know before or after deploying a package.



### Defining Documentation

To include documentation in your package, add a `documentation` section to your `zarf.yaml`:
...
Documents can be stored in the package under the top level `documentation` field, which maps documentation keys to file paths. Package deployers can view this documentation with the [zarf package inspect documentation](src/content/docs/commands/zarf_package_inspect_documentation.md) command. Common use cases for package documentation include information around the values to set on deploy, changelogs, or application specific information.

For instance, in the [dos-games](src/content/ref/examples/dos-games) package, the documentation informs users on the level 1 secrets.
