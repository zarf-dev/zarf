...
Package consumers can extract documentation using the command [zarf package inspect documentation](/commands/zarf_package_inspect_documentation). If two files have the same basename, then the key will be prepended to the duplicate filenames. View an example of the `documentation` field in the [dos-games](/ref/examples/dos-games) package.
