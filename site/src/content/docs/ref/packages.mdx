...
Documentation can be stored in the package under the top level `documentation` field, which maps documentation keys to file paths. Package consumers can view documentation using [zarf package inspect documentation](/commands/zarf_package_inspect_documentation). This is particularly useful for communicating configuration options, changelogs, or application specific information. For instance, in the [dos-games](/ref/examples/dos-games) package, the documentation informs users on the level 1 secrets.
