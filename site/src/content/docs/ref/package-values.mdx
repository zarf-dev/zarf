...
Every value referenced by a Go template must be explicitly defined, either in the package's values files or provided at deploy time, even if its value is empty. Deployments fail early on missing keys.
