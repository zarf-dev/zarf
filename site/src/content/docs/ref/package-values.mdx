...
Package Values provide a familiar way to define configuration data in your Zarf packages. Unlike the legacy [Variables and Constants](/ref/values/) system that uses string substitution with `###ZARF_VAR_###` syntax, Package Values uses [Go templates](https://pkg.go.dev/text/template) with access to structured data and custom functions like those offered by [Sprig](/ref/templating/).
