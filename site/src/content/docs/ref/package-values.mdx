...
Package Values provide a familiar way to define configuration data in your Zarf packages. Unlike the legacy [Variables and Constants](/ref/values/) system that uses string substitution with `###ZARF_VAR_###` syntax, Package Values uses [Go templates](https://pkg.go.dev/text/template) with access to structured data and custom functions like those offered by [Sprig](#functions).

:::note

Package Values is a **beta** feature. It is ready for realistic workloads and user feedback, but its API may still change before general availability. Zarf will document any breaking change and migration guidance.
...
Every key referenced by the package must be explicitly defined, either in the package's values files or provided at deploy time. A key can be defined as empty, but deploy is blocked if any key is missing.
...
Templating can be enabled during `onDeploy` and `onRemove` actions, but **not** during `onCreate` actions.
...
`--set-values` accepts `key.path=value` pairs with nested keys (`a.b.c=value`). Values are type-inferred: `true`/`false` become booleans and whole base-10 numbers become integers. Everything else, such as decimals (`1.5`) and leading-zero numbers (`0755`), stays a string. To force a value that would otherwise be inferred to stay a string, wrap it in single quotes, e.g. `--set-values="version='123'"`. Lists and other types are not supported through `--set-values`; provide them through a `--values` file instead.
...
The `.Pkg` object gives access to the package definition. See this used in the [values-templating](/ref/examples/values-templating) example.

### Functions

[sprig functions](https://masterminds.github.io/sprig/) except for `getHostByName`, `env`, and `expandenv` are included.
...
- [Variables to Values](/best-practices/variables-to-values/) - Migrating from the legacy Variables and Constants system
- [Deployment Variables (Legacy)](/ref/values/) - Legacy Variables and Constants system
