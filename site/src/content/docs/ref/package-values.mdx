...
`--set-values` uses [Helm's `--set` format](https://helm.sh/docs/intro/using_helm/#the-format-and-limitations-of---set), including nested keys (`a.b.c=value`), lists (`items='{x,y,z}'`), and list indexing (`items[0]=x`). Scalars are type-inferred: `true`/`false` become booleans, `null` becomes null, and whole base-10 numbers become integers. Everything else such as decimals (`1.5`) and leading-zero numbers (`0755`) stays a string. To set a value whose type the flag won't infer, such as a float, provide it through a `--values` file where YAML determines the type.
