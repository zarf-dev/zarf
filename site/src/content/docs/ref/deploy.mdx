...
- **Connected Mode** - Deploying with the `--connected` flag skips the publishing of Git repositories and container images. Deploying directly from an OCI source also skips pulling the package's container images. Connected mode does not require `zarf init` to be run, if cluster is already initialized then mutations will be prevented by labeling resources with `zarf.dev/agent: ignore`.
