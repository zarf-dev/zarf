...
- Not require `zarf init` to be run
- Be able to create+deploy a package in either connected mode (default) or prod mode (exposed via `--connected=false`)
...
```bash
# Create and deploy dos-games in connected mode
...
$ zarf dev deploy examples/dos-games --connected=false
```
