...
For component selection, `zarf init` reads `init.components` and `zarf package deploy` reads `package.deploy.components`.
If `init.components` is not set, `zarf init` also accepts `package.deploy.components` as a backward-compatible fallback.
