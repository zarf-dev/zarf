...
Helm templating happens inside Helm charts (the standard Helm behavior). Use [Helm value mappings](/ref/package-values/#mapping-to-helm-chart-values) to pass Zarf values into Helm charts so they may be accessed in Helm chart templates.
