...
Zarf includes the full [Sprig template function library](http://masterminds.github.io/sprig/), providing powerful data transformation capabilities.
...
name: {{ .Values.app.name | lower }}        # my-app
name: {{ .Values.app.name | title }}        # My-App

# Format conversion
name: {{ .Values.site.title | kebabcase }}  # my-awesome-app
name: {{ .Values.site.title | snakecase }}  # my_awesome_app
name: {{ .Values.site.title | camelcase }}  # myAwesomeApp

# Quoting
value: {{ .Values.app.name | quote }}       # "my-app"
value: {{ .Values.app.name | squote }}      # 'my-app'

# Trimming and padding
message: {{ .Values.message | trim }}
message: {{ .Values.message | trimPrefix "prefix-" }}
message: {{ .Values.message | trimSuffix "-suffix" }}

# Repetition and truncation
dashes: {{ "-" | repeat 40 }}               # ----------------------------------------
short: {{ .Values.longText | trunc 50 }}    # First 50 characters
```
...
# Output: "templating, sprig-functions, kubernetes-deployment"

# List properties
count: {{ .Values.app.features | len }}              # 3
first: {{ .Values.app.features | first }}            # templating
last: {{ .Values.app.features | last }}              # kubernetes-deployment

# Sorting and reversing
sorted: {{ .Values.app.features | sortAlpha | join ", " }}
reversed: {{ .Values.app.features | reverse | join ", " }}

# Checking membership
{{ if has "templating" .Values.app.features }}
  # templating feature is enabled
{{ end }}
```
...
metadata:
  name: app-config
data:
  features: {{ .Values.app.features | join "," | quote }}
  feature-count: {{ .Values.app.features | len | toString | quote }}
  first-feature: {{ .Values.app.features | first | quote }}
  sorted-features: {{ .Values.app.features | sortAlpha | join "," | quote }}
```

### Default and Conditional Functions

Provide fallback values and conditional logic:
  ...
  template:
    spec:
      ...
      - name: app
        ...
        - name: DEBUG
          value: {{ .Values.app.debug | default false | quote }}
```
...
```yaml
# Basic arithmetic
sum: {{ add .Values.app.replicas 2 }}           # replicas + 2
product: {{ mul .Values.app.replicas 10 }}      # replicas * 10
difference: {{ sub .Values.app.replicas 1 }}    # replicas - 1
quotient: {{ div 100 .Values.app.replicas }}    # 100 / replicas

# Min/Max
max-port: {{ max .Values.app.port1 .Values.app.port2 .Values.app.port3 }}
min-port: {{ min .Values.app.port1 .Values.app.port2 .Values.app.port3 }}

# Rounding
rounded: {{ .Values.app.price | round 2 }}      # Round to 2 decimals
ceiling: {{ .Values.app.price | ceil }}
floor: {{ .Values.app.price | floor }}
```
...
data:
  ...
  replicas-plus-two: {{ add .Values.app.replicas 2 | quote }}
  scaled-replicas: {{ mul .Values.app.replicas 10 | quote }}
  max-port: {{ max .Values.app.port1 .Values.app.port2 | quote }}
```

### Encoding and Hashing Functions

Encode data and generate hashes:

```yaml
# Base64 encoding/decoding
encoded: {{ .Values.app.secret | b64enc }}
decoded: {{ .Values.app.encodedSecret | b64dec }}

# Hashing
sha256: {{ .Values.app.data | sha256sum }}
sha1: {{ .Values.app.data | sha1sum }}

# URL encoding
url: {{ .Values.app.callback | urlquery }}
```
...
data:
  ...
  config-hash: {{ .Values.app.config | sha256sum | b64enc }}
```

### Formatting Functions

Like Helm, Zarf offers functions for formatting and indenting output:

```yaml
# Indentation
{{ .Values.multilineConfig | indent 4 }}
{{ .Values.multilineConfig | nindent 2 }}  # newline + indent

# Type conversion
port: {{ .Values.app.port | toString }}
count: {{ .Values.app.count | toInt }}
...
**Example with multi-line values:**
...
data:
  config.yaml: |
    # Indent multi-line YAML
{{ .Values.app.config | toYaml | indent 4 }}

  styles.css: |
{{ .Values.site.styles | indent 4 }}
```
...
Combining multiple Sprig functions:
...
metadata:
  ...
  labels:
    ...
    environment: {{ .Values.app.environment | upper }}
data:
  # Multi-line HTML with various transformations
  index.html: |
    <!DOCTYPE html>
    <html>
      <head>
        <title>{{ .Values.site.name | title }}</title>
        <style>
{{ .Values.site.styles | indent 10 }}
        </style>
      </head>
      <body>
        <h1>{{ .Values.site.name | title }}</h1>

        <!-- String transformations -->
        <p>Message: {{ .Values.site.message | upper | quote }}</p>
        <p>Slug: {{ .Values.site.title | kebabcase }}</p>

        <!-- List operations -->
        <p>Features: {{ .Values.app.features | join ", " }}</p>
        <p>Feature Count: {{ .Values.app.features | len }}</p>
        <p>First Feature: {{ .Values.app.features | first | title }}</p>

        <!-- Default values -->
        <p>Log Level: {{ .Values.app.logLevel | default "info" | upper }}</p>

        <!-- Math operations -->
        <p>Replicas: {{ .Values.app.replicas }}</p>
        <p>Scaled: {{ mul .Values.app.replicas 10 }}</p>
        <p>Max Port: {{ max .Values.app.port1 .Values.app.port2 }}</p>

        <!-- Encoding -->
        <p>Config Hash: {{ .Values.app.config | sha256sum }}</p>
        <p>Encoded: {{ .Values.app.secret | b64enc }}</p>
      </body>
    </html>
```
...
See the [values-templating example](/ref/examples/) for comprehensive demonstrations of:

- Basic template syntax
- All Sprig function categories
- Conditional logic
- List iteration
