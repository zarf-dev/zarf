...
The [`zarf dev lint`](/v0.77/commands/zarf_dev_lint) command is used to validate a `zarf.yaml` file against the Zarf schema. This command is useful for ensuring that a `zarf.yaml` file is correctly formatted and adheres to the Zarf schema.
