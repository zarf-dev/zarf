...
Want to see a guided `zarf init`? Check out the [Zarf Init tutorial](/v0.77/tutorials/1-initializing-a-k8s-cluster/).

View all init options w/ [`zarf init --help`](/v0.77/commands/zarf_init/).
:::
...
2. A `connect` manifest for running [`zarf connect registry`](/v0.77/commands/zarf_connect/) to tunnel to the Zarf Registry.
...
:::tip
You can further customize how the registry behaves by setting variables such as `REGISTRY_PVC_SIZE` with a [config file](/v0.77/ref/config-files/) or `--set` on `zarf init`.
...
Zarf can be configured to use an already existing registry with the `--registry-*` flags when running [`zarf init`](/v0.77/commands/zarf_init/).
...
Additionally, you must provide custom configuration to the registry to ensure it is distributed across all nodes and has the appropriate number of replicas. Below is an example [configuration file](/v0.77/ref/config-files/) using a ReadWriteMany storage class:
...
:::tip
You can further customize how the git-server behaves by setting variables such as `GIT_SERVER_PVC_SIZE` with a [config file](/v0.77/ref/config-files/) or `--set` on `zarf init`.
...
You can learn more about creating a custom init package in the [Creating a Custom 'init' Package Tutorial](/v0.77/tutorials/7-custom-init-packages).
:::
