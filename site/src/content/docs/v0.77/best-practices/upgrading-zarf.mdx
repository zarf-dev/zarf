...
slug: v0.77/best-practices/upgrading-zarf
---

Optimally the version of Zarf performing the `package create` and `package deploy` operations aligns to the version of [Init Package](/v0.77/ref/init-package) that is installed within the cluster.

Follow [CLI Installation](/v0.77/getting-started/install) steps for installing updates to the version of binary installed on the machine executing zarf commands.
...
* **Deploy:** Init Packages can also be deployed directly with `package deploy` - allowing for explicit deployment specification and the use of [custom init packages](/v0.77/tutorials/7-custom-init-packages).
...
See the [init command](/v0.77/commands/zarf_init/#options) for reference.

In order to update any configuration passed into the `zarf init` command, you will need to run [zarf tools update-creds](/v0.77/commands/zarf_tools_update-creds/#options) with the information necessary.
