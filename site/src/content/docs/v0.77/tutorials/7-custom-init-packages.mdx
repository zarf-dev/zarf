...
sidebar:
  order: 8
slug: v0.77/tutorials/7-custom-init-packages
---
...
In this tutorial, we will demonstrate how to build a custom [Zarf 'init' Package](/v0.77/ref/init-package/) with `zarf package create`.
...
* (if building a local [`zarf-agent`](/v0.77/faq#what-is-the-zarf-agent)) The [Docker CLI](https://docs.docker.com/desktop/) installed and the tools to [Build your own CLI](/v0.77/getting-started/install/#building-from-source)
...
<iframe src="/tutorials/package_create_init.html" height="500px" width="100%" />
...
The Zarf 'init' Package is a [composed Zarf Package](/v0.77/ref/components/#component-imports) made up of many sub-Zarf Packages. The root `zarf.yaml` file is defined at the root of the Zarf git repository.
...
Registry overrides can also be orchestrated with environment variables and/or configuration files:

```bash
export ZARF_PACKAGE_CREATE_REGISTRY_OVERRIDE='docker.io=dockerio.enterprise.corp,ghcr.io=ghcr.enterprise.corp,quay.io=quay.enterprise.corp'
zarf package create .
```
...
--set REGISTRY_IMAGE_TAG=3.0.0 \
--set REGISTRY_IMAGE="opensource/registry" \
--set REGISTRY_IMAGE_DOMAIN="custom.enterprise.corp" \
--set PROXY_IMAGE_TAG="1.8.0.3" \
--set PROXY_IMAGE_DOMAIN="custom.enterprise.corp" \
--set PROXY_IMAGE="alpine/socat" \
...
You may not need or want all of the components in your 'init' package and may choose to slim down your package by removing them. Because the [Zarf Package is composed](/v0.77/ref/components/#component-imports) all you need to do is remove the component that imports the component you wish to exclude.
