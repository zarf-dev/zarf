...
No, while improving the airgapped Kubernetes UX is often the top priority for the project, many users find value in [mirroring resources](/v0.77/commands/zarf_package_mirror-resources/) across environments and using [Zarf actions](/v0.77/ref/actions/).
...
This can be done by running a local registry and pushing the images to it before running `zarf package create`. This will allow `zarf package create` to pull the images from the local registry instead of Docker. This can also be combined with [component actions](/v0.77/ref/actions/) and [`--registry-override`](/v0.77/commands/zarf_package_create/) to make the process automatic. Given an example image of `registry.enterprise.corp/my-giant-image:v2` you could do something like this:
...
## When should I clear my Zarf cache?

When Zarf pulls new images or OCI artifacts, it will store them locally to speed up future uses. Over time, the cache
will grow, especially if you run `zarf package create` often with a variety of packages or with many large images. The
default location is in the home directory for the current user at `~/.zarf-cache`. Unix users can check the cache size
with `du -sh ~/.zarf-cache` and if you'd like to free up some disk space we recommend running `zarf tools clear-cache`.
How often you clear your cache will depend on your disk space and network needs.
