...
// LintChain lints the chain of nodes
...
func (node *Node) lint() []lint.ValidatorMessage {
	...
	findings = append(findings, node.checkForDeprecatedScripts()...)
	findings = append(findings, node.checkForSetVariable()...)
	for idx := range findings {
		findings[idx].PackageName = node.OriginalPackageName()
		findings[idx].PackageRelPath = node.ImportLocation()
	}
	return findings
}
...
func (node *Node) checkForDeprecatedScripts() []lint.ValidatorMessage {
	findings := []lint.ValidatorMessage{}
	if len(node.DeprecatedScripts.After) > 0 || len(node.DeprecatedScripts.Before) > 0 ||
		len(node.DeprecatedScripts.Prepare) > 0 {
		findings = append(findings, lint.ValidatorMessage{
			YqPath:      fmt.Sprintf(".components.[%d]", node.Index()),
			Description: "Using scripts which will be removed in Zarf v1.0.0. Please migrate to actions.",
			Item:        node.Import.URL,
			Category:    lint.CategoryWarning,
		})
	}
...
func (node *Node) checkForSetVariable() []lint.ValidatorMessage {
	findings := []lint.ValidatorMessage{}
	if len(node.DeprecatedScripts.After) > 0 || len(node.DeprecatedScripts.Before) > 0 ||
		len(node.DeprecatedScripts.Prepare) > 0 {
		findings = append(findings, lint.ValidatorMessage{
			YqPath:      fmt.Sprintf(".components.[%d]", node.Index()),
			Description: "Using setVariable in actions which will be removed in Zarf v1.0.0. Please migrate to the list form of setVariables",
			Item:        node.Import.URL,
			Category:    lint.CategoryWarning,
		})
	}
