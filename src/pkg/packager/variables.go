...
func (p *Packager) setVariableMapInConfig() error {
	for name, value := range p.cfg.PkgOpts.SetVariables {
		p.cfg.SetVariableMap.SetVariableInConfig(name, value, false, false, "")
	}
	...
	for _, variable := range p.cfg.Pkg.Variables {
		...
		if present {
			...
			if err := p.cfg.SetVariableMap.CheckVariablePattern(variable.Name, variable.Pattern); err != nil {
				return err
			}
			continue
		}
		...
		p.cfg.SetVariableMap.SetVariableInConfig(variable.Name, variable.Default, variable.Sensitive, variable.AutoIndent, variable.Type)
		...
		if variable.Prompt && !config.CommonOptions.Confirm {
			...
			p.cfg.SetVariableMap.SetVariableInConfig(variable.Name, val, variable.Sensitive, variable.AutoIndent, variable.Type)
		}
