...
// Level declares each support log level. These are 1:1 what log/slog supports by default. Conveniently, an empty log
// level maps to 0 which is useful for a default.
type Level int
...
// KeyLevel is a lowercase string representation of a log Level.
type KeyLevel string

var (
	KeyDebug KeyLevel = "debug"
	KeyInfo  KeyLevel = "info"
	KeyWarn  KeyLevel = "warn"
	KeyError KeyLevel = "error"
)

// keyLevels maps a string Key to its Level.
var keyLevels = map[KeyLevel]Level{
	KeyDebug: Debug,
	KeyInfo:  Info,
	KeyWarn:  Warn,
	KeyError: Error,
...
// StringKeyToLevel takes a string representation of a Key, ensure it exists, and then converts it into a Level.
func StringKeyToLevel(key string) (Level, error) {
	k := KeyLevel(strings.ToLower(key))
	l, ok := keyLevels[k]
	if !ok {
...
func New(cfg Config) (*slog.Logger, error) {
	...
	//		AddSource: true,
	//	})
	// FIXME(mkcp): We probably want --disable-logs to be its own deal
	case FormatNone:
		handler = slog.NewTextHandler(DestinationNone, &slog.HandlerOptions{})
