...
// CosignSignImageWithOptions signs an OCI artifact by its immutable digest and uploads the
// signature to its registry.
func CosignSignImageWithOptions(ctx context.Context, image string, opts SignBlobOptions, plainHTTP, insecureSkipTLSVerify bool) error {
	l := logger.From(ctx)
	...
	if err := signcommon.LoadTrustedMaterialAndSigningConfig(ctx, &ko,
		opts.UseSigningConfig, "", opts.Rekor.URL, opts.Fulcio.URL, opts.OIDC.Issuer, opts.TSAServerURL, "",
		opts.TlogUpload, opts.NewBundleFormat, "", opts.Key, false,
		"", "", "", "", "", ""); err != nil {
		return err
	}
	...
	signOpts := options.SignOptions{
		...
		TSAServerURL:     opts.TSAServerURL,
		NewBundleFormat:  opts.NewBundleFormat,
		UseSigningConfig: opts.UseSigningConfig,
		Rekor:            opts.Rekor,
		Fulcio:           opts.Fulcio,
		OIDC:             opts.OIDC,
	...
	l.Debug("signing OCI artifact with cosign", "reference", image)
	if err := sign.SignCmd(ctx, rootOpts, ko, signOpts, []string{image}); err != nil {
		return err
	}
	l.Debug("OCI artifact signed successfully", "reference", image)
	return nil
}
