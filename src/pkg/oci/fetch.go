...
func (o *OrasRemote) FetchRoot() (*Manifest, error) {
	if o.root != nil {
		return o.root, nil
	}
...
func (o *OrasRemote) FetchManifest(desc ocispec.Descriptor) (manifest *Manifest, err error) {
	return FetchUnmarshal[*Manifest](o.FetchLayer, json.Unmarshal, desc)
}
...
func FetchJSONFile[T any](fetcher func(desc ocispec.Descriptor) (bytes []byte, err error), manifest *Manifest, path string) (result T, err error) {
...
func FetchYAMLFile[T any](fetcher func(desc ocispec.Descriptor) (bytes []byte, err error), manifest *Manifest, path string) (result T, err error) {
	descriptor := manifest.Locate(path)
	if IsEmptyDescriptor(descriptor) {
		return result, fmt.Errorf("unable to find %s in the manifest", path)
	}
