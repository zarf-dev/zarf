...
func GenerateSBOM(ctx context.Context, buildPath string, images []transform.Image) error {
	...
	for _, refInfo := range images {
		...
		err = createSBOMViewerAsset(refInfo.Reference, b, jsonList)
		if err != nil {
			return err
		}
	}
...
func createSBOMViewerAsset(identifier string, jsonData, jsonList []byte) error {
	filename := fmt.Sprintf("sbom-viewer-%s.html", getNormalizedFileName(identifier))
	return createSBOMHTML(filename, "viewer/template.gohtml", jsonData, jsonList)
}

func createSBOMCompareAsset() error {
	return createSBOMHTML("compare.html", "viewer/compare.gohtml", nil, nil)
}

func createSBOMHTML(filename string, goTemplate string, jsonData []byte, jsonList []byte) error {
	// path := filepath.Join(outputDir, getNormalizedFileName(filename))
	path := filepath.Join(getNormalizedFileName(filename))
