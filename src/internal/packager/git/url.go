...
var gitURLRegex = regexp.MustCompile(`^(?P<proto>[a-z]+:\/\/)(?P<hostPath>.+?)\/(?P<repo>[\w\-\.]+?)(?P<git>\.git)?(?P<atRef>@(?P<ref>[\w\-\.]+))?(?P<gitPath>\/(?P<gitPathId>info\/.*|git-upload-pack|git-receive-pack))?$`)
...
func (g *Git) TransformURL(url string) (string, error) {
	...
	output := fmt.Sprintf("%s/%s/%s%s%s", g.Server.Address, g.Server.PushUsername, repoName, matches[idx("git")], matches[idx("gitPath")])
