...
func TestComponentChoice(t *testing.T) {
	...
	stdOut, stdErr, err := e2e.Zarf("package", "deploy", path, "--confirm", "--components=first-choice,second-choice")
	require.Error(t, err, stdOut, stdErr)

	// Deploy a single choice and expect success
	stdOut, stdErr, err = e2e.Zarf("package", "deploy", path, "--confirm", "--components=first-choice")
