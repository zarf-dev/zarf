...
func TestPublishV1Beta1Component(t *testing.T) {
	...
	// FIXME: We should verify that the manifest has all the paths we expect. It should have one of each resource
	// and the remote version of that resource
	require.NoError(t, json.Unmarshal(manifestBytes, &manifest))
	require.Equal(t, packager.ComponentConfigMediaType, manifest.Config.MediaType)
	require.NotEmpty(t, manifest.Layers)
	require.Contains(t, manifest.Layers[0].Annotations["org.opencontainers.image.title"], "manifest.yaml")
}
