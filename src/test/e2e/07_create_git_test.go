...
func TestCreateGit(t *testing.T) {
	...
	stdOut, stdErr, _, err := e2e.ExecZarfCommand("tools", "archiver", "decompress", path, extractDir, "--decompress-all")
	...
	stdOut, stdErr, _, err = exec.Cmd("git", gitDirFlag, "log", "--oneline", "--decorate")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = exec.Cmd("git", gitDirFlag, "log", "HEAD^..HEAD", "--oneline", "--decorate")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = exec.Cmd("git", gitDirFlag, "tag")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = exec.Cmd("git", gitDirFlag, "branch")
	require.NoError(t, err, stdOut, stdErr)
