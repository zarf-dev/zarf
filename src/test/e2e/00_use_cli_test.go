...
func TestUseCLI(t *testing.T) {
	...
	stdOut, stdErr, _, err := e2e.ExecZarfCommand("prepare", "sha256sum", shasumTestFilePath)
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("prepare", "sha256sum", "https://zarf-public.s3-us-gov-west-1.amazonaws.com/pipelines/zarf-prepare-shasum-remote-test-file.txt")
	assert.NoError(t, err, stdOut, stdErr)
	assert.Contains(t, stdOut, expectedShasum, "The expected SHASUM should equal the actual SHASUM")

	// Test `zarf version`
	stdOut, _, _, err = e2e.ExecZarfCommand("version")
	assert.NoError(t, err)
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("prepare", "find-images", "examples/helm-alt-release-name")
	assert.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("prepare", "find-images", "examples/helm-local-chart")
	assert.NoError(t, err, stdOut, stdErr)
	...
	_, _, _, err = e2e.ExecZarfCommand("init", "--confirm", "--components=k3s,foo,logging")
	assert.Error(t, err)

	// Test that changing the log level actually applies the requested level
	_, stdErr, _, _ = e2e.ExecZarfCommand("version", "--log-level=debug")
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "deploy", "https://zarf-examples.s3.amazonaws.com/zarf-package-appliance-demo-doom-20210125.tar.zst", "--confirm")
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "create", "examples/dos-games", "--confirm", "--zarf-cache", cachePath, "--tmpdir", otherTmpPath, "--log-level=debug")
	require.Contains(t, stdErr, otherTmpPath, "The other tmp path should show as being created")
	require.NoError(t, err, stdOut, stdErr)

	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "inspect", pkgName, "--tmpdir", otherTmpPath, "--log-level=debug")
	require.Contains(t, stdErr, otherTmpPath, "The other tmp path should show as being created")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("tools", "clear-cache", "--zarf-cache", cachePath)
	require.NoError(t, err, stdOut, stdErr)
	...
	tlsCert := "tls.crt"
	tlsKey := "tls.key"
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("tools", "gen-pki", "github.com", "--sub-alt-name", "google.com")
	require.NoError(t, err, stdOut, stdErr)
