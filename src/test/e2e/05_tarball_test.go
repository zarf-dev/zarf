...
func TestReproducibleTarballs(t *testing.T) {
	...
	var (
		...
		tmp        = t.TempDir()
		tb         = filepath.Join(tmp, fmt.Sprintf("zarf-package-dos-games-%s-1.3.0.tar.zst", e2e.Arch))
...
func TestPackageTarballDirectoryStructure(t *testing.T) {
	...
	tests := []struct {
		...
		createArgs        []string
		wantBlobs         int
		wantComponentDirs []string
	}{
		// Each test is about ~15 seconds - only testing required permutations
		{
			name:              "archive structure",
			createPath:        filepath.Join("src", "test", "packages", "05-archive-structure"),
			packageName:       fmt.Sprintf("zarf-package-test-archive-structure-%s-0.0.1.tar.zst", e2e.Arch),
			createArgs:        []string{"--confirm"},
			wantBlobs:         17,
			wantComponentDirs: []string{"test-component-1", "test-component-2"},
		},
		{
			name:              "archive structure by arbitrary filename and imports",
			createPath:        filepath.Join("src", "test", "packages", "05-arbitrary-archive-structure", "arbitrary.yaml"),
			packageName:       fmt.Sprintf("zarf-package-test-archive-structure-arbitrary-%s-0.0.1.tar.zst", e2e.Arch),
			createArgs:        []string{"--confirm"},
			wantBlobs:         17,
			wantComponentDirs: []string{"test-component-1", "test-component-2"},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tmp := t.TempDir()
			tb := filepath.Join(tmp, tt.packageName)
			unpack := filepath.Join(tmp, "unpack")
			unpackAll := filepath.Join(tmp, "unpack-all")

			createArgs := append([]string{"package", "create", tt.createPath}, tt.createArgs...)
			createArgs = append(createArgs, "--output", tmp)
			stdOut, stdErr, err := e2e.Zarf(t, createArgs...)
			require.NoError(t, err, stdOut, stdErr)
			...
			// Check that images/blobs/sha256 contains the expected number of files
			blobDir := filepath.Join(unpack, "images", "blobs", "sha256")
			entries, err := os.ReadDir(blobDir)
			require.NoError(t, err, "failed to read sha256 dir %q", blobDir)

			if len(entries) != tt.wantBlobs {
				require.Fail(t, fmt.Sprintf("expected %d entries in %q, but found %d", tt.wantBlobs, blobDir, len(entries)))
			}
			...
			// Check the --unarchive-all option - performing this in-line to avoid re-creating the tarball
			// |-- checksums.txt
			// |-- components
			// |   |-- test-component-1
			// |   |   |-- charts
			...
			// `-- zarf.yaml
			for _, comp := range tt.wantComponentDirs {
