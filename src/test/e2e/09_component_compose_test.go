...
func (suite *CompositionSuite) Test_0_ComposabilityExample() {
	...
	_, _, err := e2e.Zarf(suite.T(), "package", "create", composeExample, "-o", "build", "--no-color", "--confirm")
	suite.NoError(err)
...
func (suite *CompositionSuite) Test_1_FullComposability() {
	...
	_, _, err := e2e.Zarf(suite.T(), "package", "create", composeTest, "-o", "build", "--no-color", "--confirm")
	suite.NoError(err)
