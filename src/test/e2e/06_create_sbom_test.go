...
func TestCreateSBOM(t *testing.T) {
	...
	_, err = os.ReadFile(filepath.Join(sbomPath, "init", "sbom-viewer-docker.io_gitea_gitea_1.20.5-rootless.html"))
	require.NoError(t, err)
	_, err = os.ReadFile(filepath.Join(sbomPath, "init", "docker.io_gitea_gitea_1.20.5-rootless.json"))
	require.NoError(t, err)
