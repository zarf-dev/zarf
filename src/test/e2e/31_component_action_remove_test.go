...
func TestComponentActionRemove(t *testing.T) {
	...
	stdOut, stdErr, _, err := e2e.ExecZarfCommand("package", "deploy", path, "--confirm", "--components=on-remove")
	require.NoError(t, err, stdOut, stdErr)

	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "remove", path, "--confirm", "--components=on-remove")
	require.NoError(t, err, stdOut, stdErr)
