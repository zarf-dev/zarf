...
func TestCreateDifferential(t *testing.T) {
	...
	stdOut, stdErr, err := e2e.ZarfWithConfirm("package", "create", packagePath, "--set=PACKAGE_VERSION=v0.25.0")
	require.NoError(t, err, stdOut, stdErr)
	defer e2e.CleanFiles(packageName)

	// Build the differential package without changing the version
	_, stdErr, err = e2e.ZarfWithConfirm("package", "create", packagePath, "--set=PACKAGE_VERSION=v0.25.0", differentialFlag)
	...
	_, stdErr, err = e2e.ZarfWithConfirm("package", "create", packagePath, "--set=PACKAGE_VERSION=v0.26.0", differentialFlag)
	require.NoError(t, err, stdOut, stdErr)
