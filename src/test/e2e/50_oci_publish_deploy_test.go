...
func (suite *PublishDeploySuiteTestSuite) Test_3_Copy() {
	...
	src, err := zoci.NewRemote(ref, oci.PlatformForArch(e2e.Arch), oci.WithInsecure(true))
	suite.NoError(err)

	dst, err := zoci.NewRemote(dstRef, oci.PlatformForArch(e2e.Arch), oci.WithInsecure(true))
	suite.NoError(err)
