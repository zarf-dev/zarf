...
func (suite *PublishDeploySuiteTestSuite) Test_3_Copy() {
	...
	src, err := oci.NewOrasRemote(ref, oci.WithArch(e2e.Arch), oci.WithPlainHTTP(true))
	suite.NoError(err)

	dst, err := oci.NewOrasRemote(dstRef, oci.WithArch(e2e.Arch), oci.WithPlainHTTP(true))
	suite.NoError(err)
