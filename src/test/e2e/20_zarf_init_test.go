...
func TestZarfInit(t *testing.T) {
	...
	// run `zarf init`
	_, initStdErr, err := e2e.Zarf("init", "--components="+initComponents, "--set", "REGISTRY_STORAGE_CLASS=local-path", "--set", "REGISTRY_NODEPORT=31337", "-l", "trace", "--confirm")
	require.NoError(t, err)
	...
	if e2e.ApplianceMode {
		...
		require.NoError(t, err)
		require.Contains(t, kubeletVersion, "v1.28.2+k3s1")
	}
