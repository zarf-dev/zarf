...
func TestDeprecatedComponentScripts(t *testing.T) {
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "deploy", testPackagePath, "--confirm", "--components=2-test-deprecated-deploy-scripts")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, _, err = e2e.ExecZarfCommand("package", "deploy", testPackagePath, "--confirm", "--components=3-test-deprecated-timeout-scripts")
