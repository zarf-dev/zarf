...
func (suite *SkeletonSuite) SetupSuite() {
	err := exec.CmdWithPrint("cp", fmt.Sprintf("-r %s %s", filepath.Join("examples", "helm-local-chart", "chart"), filepath.Join("src", "test", "test-packages", "51-import-everything", "charts", "local")))
	suite.NoError(err)
