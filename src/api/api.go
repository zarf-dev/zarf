...
func (p PackageDefinition) filterView() filters.PackageView {
	definitions := p.componentDefinitionsForDisplay()
	components := make([]filters.ComponentView, 0, len(p.pkg.Components))
	for idx, component := range p.pkg.Components {
...
func (p PackageDefinition) componentDefinitionsForDisplay() []any {
	definitions := make([]any, len(p.pkg.Components))
	if p.OriginalAPIVersion() == v1beta1.APIVersion {
		for idx, component := range p.AsV1beta1().Components {
			definitions[idx] = component
		}
		return definitions
	}
	for idx, component := range p.AsV1alpha1().Components {
		definitions[idx] = component
	}
	return definitions
}
