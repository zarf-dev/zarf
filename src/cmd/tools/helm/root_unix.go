...
func checkPerms() {
	...
	perm := fi.Mode().Perm()
	if perm&0040 > 0 {
		message.Warnf("Kubernetes configuration file is group-readable. This is insecure. Location: %s", kc)
	}
	if perm&0004 > 0 {
		message.Warnf("Kubernetes configuration file is world-readable. This is insecure. Location: %s", kc)
	}
}
