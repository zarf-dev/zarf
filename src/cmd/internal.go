	...
	for _, cmd := range o.rootCmd.Commands() {
		if cmd.Use == "tools" {
			for _, toolCmd := range cmd.Commands() {
				...
				if toolCmd.Use == "yq" {
					for _, subCmd := range toolCmd.Commands() {
						// yq's vendored completion Long text is flush-left shell
						// examples that the docs site's MDX parser can't render.
						if subCmd.Name() == "shell-completion" || subCmd.Name() == "completion" {
							subCmd.Hidden = true
						}
					}
				}
...
`, title, title)
	...
	const commandsDir = "./site/src/content/docs/commands"
	if err := doc.GenMarkdownTreeCustom(o.rootCmd, commandsDir, prependTitle, linkHandler); err != nil {
		return err
	}

	// The docs site renders these files as MDX, which does not support indented
	// code blocks. Convert them to fenced blocks so cobra synopsis examples
	// (e.g. `source <(zarf completion zsh)`) don't break the MDX parser.
	...
	for _, entry := range entries {
		...
		p := filepath.Join(commandsDir, entry.Name())
		b, err := os.ReadFile(p)
		if err != nil {
			return err
		}
		if err := os.WriteFile(p, []byte(fenceIndentedCodeBlocks(string(b))), 0o644); err != nil {
			return err
		}
	}
	return nil
...
// fenceIndentedCodeBlocks rewrites Markdown indented code blocks as fenced code
// blocks, leaving existing fenced blocks untouched. MDX, used by the docs site,
// does not support indented code blocks.
func fenceIndentedCodeBlocks(md string) string {
	isIndented := func(s string) bool {
		return strings.HasPrefix(s, "\t") || strings.HasPrefix(s, "    ")
	}
	deindent := func(s string) string {
	...
	for i := 0; i < len(lines); i++ {
		...
		if inFence || !prevBlank || !isIndented(line) || strings.TrimSpace(line) == "" {
			out = append(out, line)
			continue
		}
		...
		for i < len(lines) {
			...
			block = append(block, deindent(l))
			i++
		}
		i-- // step back so the loop re-examines the terminating line
		end := len(block)
