...
func init() {
	...
	v.SetDefault(V_CONN_NAME, "")
	v.SetDefault(V_CONN_NAMESPACE, k8s.ZarfNamespace)
	v.SetDefault(V_CONN_RESOURCE_TYPE, k8s.SvcResource)
	v.SetDefault(V_CONN_LOCAL_PORT, 0)
	v.SetDefault(V_CONN_REMOTE_PORT, 0)
	v.SetDefault(V_CONN_CLI_ONLY, false)

	connectCmd.Flags().StringVar(&connectResourceName, "name", v.GetString(V_CONN_NAME), "Specify the resource name.  E.g. name=unicorns or name=unicorn-pod-7448499f4d-b5bk6")
	connectCmd.Flags().StringVar(&connectNamespace, "namespace", v.GetString(V_CONN_NAMESPACE), "Specify the namespace.  E.g. namespace=default")
	connectCmd.Flags().StringVar(&connectResourceType, "type", v.GetString(V_CONN_RESOURCE_TYPE), "Specify the resource type.  E.g. type=svc or type=pod")
	connectCmd.Flags().IntVar(&connectLocalPort, "local-port", v.GetInt(V_CONN_LOCAL_PORT), "(Optional, autogenerated if not provided) Specify the local port to bind to.  E.g. local-port=42000")
	connectCmd.Flags().IntVar(&connectRemotePort, "remote-port", v.GetInt(V_CONN_REMOTE_PORT), "Specify the remote port of the resource to bind to.  E.g. remote-port=8080")
	connectCmd.Flags().BoolVar(&cliOnly, "cli-only", v.GetBool(V_CONN_CLI_ONLY), "Disable browser auto-open")
}
