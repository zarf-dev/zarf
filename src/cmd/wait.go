...
// newWaitForCommand creates the deprecated `wait-for` command.
func newWaitForCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Deprecated: "use 'zarf tools wait-for-resource' or 'zarf tools wait-for-network' instead",
		RunE:       o.run,
	}
...
func newWaitForNetworkCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "wait-for-network PROTOCOL ADDRESS [CODE]",
