...
Note: this code was copied from https://github.com/x-cray/logrus-prefixed-formatter (which wasn't updated in 5 years and needed adjustments)
...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	if prefixValue, ok := entry.Data["prefix"]; ok {
		val, ok := prefixValue.(string)
		if ok {
			prefix = colorScheme.PrefixColor(" " + val + ":")
		}
	} else {
