...
In most usage scenarios ([but not all](../../examples/yolo/README.md)) you will also need an ['init' package](../4-create-a-zarf-package/3-zarf-init-package.md) to be initialized before you deploy your own packages. This is a special Zarf package that initializes a cluster with services that are used to store package resources while in the air gap.
